...
`float`, `boolean`, `array`, `union`, `variant`, `nullable`, `parse`,
`parse_file`, `null`, `gen_doc`, `schema_doc`, `true`, `false`, and `in`.
...
directive_pair
  ...
  | "parse_file" , ws0 , "=" , ws0 , string_literal ;
...
Parse directives make host-provided runtime input available as typed variables
inside the output block. `parse = Schema` validates the host input record
against the locally declared schema named `Schema` and exposes each matching
schema field as a variable. `parse_file = "path"` loads schema declarations
from the referenced Mace file, then validates the host input against the schema
named by an accompanying `schema = Name` directive, and exposes the matching
fields as variables. `parse` and `parse_file` cannot be combined.
...
Parse directives deliver runtime input to the output block. When `parse = Schema` is present, the host-provided input record is validated against the named schema and each schema field is introduced as a variable into the output scope. When `parse_file = "path"` is used together with `schema = Name`, schema declarations are loaded from the referenced file and the same input validation and variable introduction applies. If the input does not satisfy the schema, processing fails with a validation error.
...
<Aside type="note" title="`mace.syntax.union-type-missing-opening-bracket`">
...
<Aside type="note" title="`mace.syntax.union-type-missing-closing-bracket`">
...
<Aside type="note" title="`mace.syntax.invalid-array-access-index`">
...
<Aside type="note" title="`mace.syntax.array-access-missing-closing-bracket`">
